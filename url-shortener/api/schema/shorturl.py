...
class ShortURLSchema(Schema):
    ...
    def validate_url(self, data, **kwargs):
        try:
            uni_url = urlparse(data["short_url"])
            ...
            ascii_url = urlparse(data["short_url"])
        ...
        if ascii_fragment:
            if len(ascii_fragment) > 1:
                fragment_sanitize = f"#{uni_fragment}"

        # url after sanitize
        data["url"] = f"{scheme_sanitize}://{host_sanitize}{port_sanitize}{path_sanitize}{query_sanitize}{fragment_sanitize}"
